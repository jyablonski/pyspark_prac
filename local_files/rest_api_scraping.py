...
df2 = df.withColumn("games_avg", F.avg("n").over(Window.partitionBy("type")))

# new agg column grouping by 2 columns
df3 = df.withColumn(
    "games_avg", F.avg("n").over(Window.partitionBy(["type", "game_type"]))
)
...
# create new column
df4 = df3.withColumn("fake_col", F.col("n") + 35)
...
df_aggs = df3.groupby(["type", "game_type"]).agg(F.sum("n").alias("sum_n"))

# create new agg df - with multiple aggs from same `.agg` function
df_aggs = df3.groupby(["type", "game_type"]).agg(
    F.sum("n").alias("sum_n"),
    F.avg("n").alias("avg_n"),
    F.max("n").alias("max_n"),
